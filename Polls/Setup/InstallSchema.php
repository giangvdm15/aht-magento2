    ...
    public function install(\Magento\Framework\Setup\SchemaSetupInterface $setup, \Magento\Framework\Setup\ModuleContextInterface $context)
    {
        ...
        if (! $installer->tableExists('giangvu_poll_answer')) {
            $poll_answer_table = $installer->getConnection()
                ...
                ->addForeignKey($installer->getFkName('giangvu_poll_answer', 'poll_id', 'giangvu_poll_entity', 'poll_id'), 'poll_id', $installer->getTable('giangvu_poll_entity'), 'poll_id', \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE)
                ->addForeignKey($installer->getFkName('giangvu_poll_answer', 'answer_id', 'giangvu_answer_entity', 'answer_id'), 'answer_id', $installer->getTable('giangvu_answer_entity'), 'answer_id', \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE)
        ...
        if (! $installer->tableExists('giangvu_poll_customer')) {
            $poll_customer_table = $installer->getConnection()
                ...
                ->addForeignKey($installer->getFkName('giangvu_poll_customer', 'poll_id', 'giangvu_poll_entity', 'poll_id'), 'poll_id', $installer->getTable('giangvu_poll_entity'), 'poll_id', \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE)
                ->addForeignKey($installer->getFkName('giangvu_poll_customer', 'customer_id', 'customer_entity', 'entity_id'), 'customer_id', $installer->getTable('customer_entity'), 'entity_id', \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE)
                ->addForeignKey($installer->getFkName('giangvu_poll_customer', 'answer_id', 'giangvu_answer_entity', 'answer_id'), 'answer_id', $installer->getTable('giangvu_answer_entity'), 'answer_id', \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE)
